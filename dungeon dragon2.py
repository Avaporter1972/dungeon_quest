...
def main():
    ...
    def search_room(player, treasures):
        ...
        if outcome ==  'treasure':
            ...
            print(f'What luck! You found a {new_treasure}, worth {treasures[new_treasure]}gold!')
        # TODO: Update player dictionary accordingly
        else:
            player['health'] -= 2
        # TODO: Print messages describing what happened
            print(f"Alas! You fell into a trap and lost 2 health points. Your current health is now: {player['health']}")
    ...
    def check_status(player):
        ...
        # TODO: If the inventory list is not empty, print items joined by commasnventory 
        if player['inventory']:
       ...
       # TODO: Inside each room, prompt player choice using input()
        ...
        # TODO: Use if/elif to handle each choice (1–4)
                if choice == '1':
                    ...
                    if room <= 4:
                        print(f"Moving to the next room... time to explore room {room + 1}!")
                    else:
                        print("You have exploded the dungeon, well done.") 
                    break
                elif choice == '3':
                    check_status(player)
                elif choice == '4':
                    print("Quitting are we...")
                    end_game(player, treasures)
                    return
                else:
                    print('Selection must be a number between 1-4. Try again.')
